...
function setCookie (name, value, days) {
  ...
  document.cookie = name + mashiro_option.cookie_version_control + '=' + (value || '') + expires + '; path=/' + ';sameSite=none'
}
...
function nextBG () {
  ...
  $('.centerbg').css('background-image', 'url("' + `${bg}${formatZero(bgindex,2)}.jpg` + '")')
}
...
$(document).ready(function () {
  var bgindex = Math.floor(Math.random() * bg_num)
  bgindex = bgindex + 1
  $('.centerbg').css('background-image', 'url("' + `${bg}${formatZero(bgindex,2)}.jpg` + '")')
